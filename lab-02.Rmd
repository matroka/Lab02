...
author: "Team Name: Mate Roka, Xu Sun, Adi Hirasaki, Xiufeng Zhu"
date: "`r Sys.Date()`"
...
frequency_table <- plastic_data_all %>% 
  group_by(region) %>% 
  count()
frequency_table
frequency_table %>% 
...
region_means <- plastic_data_all %>%
  group_by(region) %>%
  summarise(mean_total_mismanaged = mean(total_mismanage_plastic, na.rm = TRUE)) %>%
...
plastic_data_all |>
  group_by(region) |>
  summarise(
      med_pct_mis_plastic_ocean = median(pct_mismanaged_plastic_ocean)
  ) |>
  arrange(med_pct_mis_plastic_ocean)
...
region_summary <- plastic_data_all %>%
  group_by(region) %>%
  summarize(
    avg_total_mismanage_plastic = mean(total_mismanage_plastic, na.rm = TRUE),
